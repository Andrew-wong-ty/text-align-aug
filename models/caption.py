...
from .w_aug import Waug
from .loss import SupConLoss, ContrastiveLoss, MSE
...
class Caption(nn.Module):
    ...
    def forward_predict_onStep(self, text_input,text_input_attn_mask , target, target_mask,aug=True):
        ...
        if self.config.use_res:
            text_feature = self.W_vision(self.get_text_feat(text_input,text_input_attn_mask))
            text_feature_aug = self.W_rot(text_feature)
            if aug:
                hs = self.transformer(text_feature_aug+text_feature, target, target_mask)  # [128,32,256]
            else:
                hs = self.transformer(text_feature+text_feature, target, target_mask)  # [128,32,256]
            out = self.mlp(hs.permute(1, 0, 2)) # hs.permute(1, 0, 2) shape= [32,128,256]
            return out
        else:
            text_feature = self.W_vision(self.get_text_feat(text_input,text_input_attn_mask))
